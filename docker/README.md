...
**Recommended:** private network + `SIGN_BACKEND=kms` + `KMS_URL`. Configure the KMS peer itself via [kms-secp256k1-api docs](https://github.com/Interchouette-ITC/kms-secp256k1-api/blob/dev/docs/Tutorial.md) (do not duplicate peer env here). Unset or empty `SIGN_BACKEND` means `none` (no put signer; public-demo safe). Put signing (`local`, `local-production`, `kms`) belongs on a **private network**.

## Env (API container)

Typical compose / runtime vars: `APP_ADDR`, `APP_PORT`, `CEPS_RPC_URL`, `CEPS_SSE_URL`, `CEPS_CHAIN_NAME`, `SIGN_BACKEND`, `KMS_URL`, `LOCAL_KEYS_JSON`, `LOCAL_KEYS_JSON_PRODUCTION`, `CEPS_WASM_ROOT`, `RUST_LOG`, `DOTENV_DISABLE`. See [`.env.example`](../.env.example) and the root [README](../README.md).
