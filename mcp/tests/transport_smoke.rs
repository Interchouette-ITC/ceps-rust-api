...
fn mcp_bin() -> String {
    if let Some(p) = option_env!("CARGO_BIN_EXE_ceps-rust-api-mcp") {
        return p.to_string();
    }
    let release = env!("CARGO_MANIFEST_DIR").to_string() + "/target/release/ceps-rust-api-mcp";
    let debug = env!("CARGO_MANIFEST_DIR").to_string() + "/target/debug/ceps-rust-api-mcp";
...
async fn http_initialize_and_tools_list() {
    ...
    for _ in 0..40 {
        ...
        if let Ok(resp) = probe {
            ...
            assert!(names.contains(&"ceps_api_hello"), "{names:?}");
            assert!(names.contains(&"ceps_api_cep18_install"), "{names:?}");
            assert!(names.contains(&"ceps_api_docker_run"), "{names:?}");
            assert!(
                names.len() >= 80,
...
fn stdio_initialize() {
    ...
    while std::time::Instant::now() < deadline {
        ...
        match reader.read_line(&mut line) {
            ...
            Ok(_) => {
                buf.push_str(&line);
                if buf.contains("ceps-rust-api") && buf.contains(ver) {
                    break;
                }
            }
            Err(_) => break,
